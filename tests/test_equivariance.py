...
# Tolerances are RELATIVE to the magnitude of the coefficients being compared,
# then converted to an atol per assertion (the repo's convention is atol-only).
# An absolute tolerance is the wrong tool here: these identities rescale the
# problem, so at c=100 the coefficients are 100x larger and a fixed atol turns
# into a 100x tighter relative test.  Measured locally, `fn` at c=100 shows an
# absolute error of 2.3e-6 but a relative error of 7.8e-9.
#
# `pfn` is held two orders looser than the direct solvers, and not only because
# its globbed reduced problem is worse conditioned: which observations get
# globbed is a *discrete* choice driven by the preliminary fit, so a different
# BLAS can flip it and land on a different (equally optimal) vertex.  On macOS
# this showed up as a relative error of 1.05e-6 where Linux gives ~1e-9.
RTOL = {"br": 1e-7, "fn": 1e-7, "pfn": 1e-5}


def _atol(method, expected, factor=1.0):
    """Absolute tolerance scaled to what is actually being compared."""
    scale = max(float(np.abs(np.asarray(expected)).max()), 1.0)
    return RTOL[method] * scale * factor
...
def test_scale_equivariance(data, method, tau, c):
    ...
    X, y = data
    expected = c * _fit(method, X, y, tau)
    np.testing.assert_allclose(
        _fit(method, X, c * y, tau), expected,
        atol=_atol(method, expected),
...
def test_negative_scale_flips_the_quantile(data, method, tau):
    ...
    X, y = data
    expected = -_fit(method, X, y, 1.0 - tau)
    np.testing.assert_allclose(
        _fit(method, X, -y, tau), expected,
        atol=_atol(method, expected),
...
def test_regression_equivariance(data, method, tau):
    ...
    expected = _fit(method, X, y, tau) + gamma
    np.testing.assert_allclose(
        _fit(method, X, y + X @ gamma, tau), expected,
        atol=_atol(method, expected),
...
def test_reparameterization_equivariance(data, method, tau):
    ...
    expected = np.linalg.solve(A, _fit(method, X, y, tau))
    np.testing.assert_allclose(
        _fit(method, X @ A, y, tau), expected,
        # A is inverted here, so its conditioning enters the comparison.
        atol=_atol(method, expected, factor=10.0),
...
def test_estimator_level_equivariance(data, tau):
    ...
    np.testing.assert_allclose(a.coef_, c * b.coef_, atol=_atol("fn", c * b.coef_))
    np.testing.assert_allclose(
        a.intercept_, c * b.intercept_, atol=_atol("fn", c * b.intercept_)
    )
